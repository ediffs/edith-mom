...
export default function Navbar() {
  ...
  return (
    <nav className="fixed w-full top-0 left-0 pb-4 md:pb-2 flex justify-between items-center bg-[#2d2643]">
      <Link
        href="/"
      ...
      <ul className="flex text-lg p-6 justify-end items-center gap-10 text-[#ff99c9]">
        {items.map((item) => (
          <li key={item.id}>
            <Link
              href={item.href}
              className="transform scale-100 hover:scale-120 bg-indigo-950 hover:text-cyan-400 text-[#ff99c9] py-2 px-4 rounded"
            >
              {item.label}
            </Link>
          </li>
